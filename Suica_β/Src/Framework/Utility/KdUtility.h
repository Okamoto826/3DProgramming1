...
constexpr int TEN_NUM = 10;		// pow(10,1)
constexpr int KEY = 0x8000;

constexpr float MinColor = 0.f;
constexpr float MaxColor = 1.f;

// 計算
constexpr float HALF = 0.5f;
constexpr float TWICE = 2.f;

// 初期化
static const Math::Vector2 ResetScale = { 1.f,1.f };



// N 桁目の数字を返す関数
constexpr int GetDigit(const int a_num, const int digit)
{
	if (a_num <  ZERO) { return ZERO; }
	if (digit <= ZERO) { return ZERO; }
	else if (digit == ONE) { return a_num % TEN_NUM; }

	return a_num % (int)pow(TEN_NUM, digit) / (int)pow(TEN_NUM, digit + BACK);
}

constexpr float MaxDeg = 360.f;
constexpr float MinDeg = 0.f;

// 角度を0～360内に正規化する関数
constexpr void DegNormalize(float* a_deg)
{
	if		(*a_deg >= MaxDeg) { *a_deg -= MaxDeg; }
	else if (*a_deg < MinDeg)  { *a_deg += MaxDeg; }
}
