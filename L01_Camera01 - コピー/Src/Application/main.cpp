...
void Application::Update()
{
	//カメラ行列の更新
	{
		...
		//回転行列
		Math::Matrix _mRotateY=
			Math::Matrix::CreateRotationY(DirectX::XMConvertToRadians(rad));
	

		//カメラのワールド行列を作成し、
		Math::Matrix _mWorld = _mScale  * _mTrans* _mRotateY;
...
void Application::Draw()
{
	...
	KdShaderManager::Instance().m_StandardShader.BeginLit();
	{
		...
		//_mat._43 = 5.0;
		//Math::Matrix _rad = Math::Matrix::CreateRotationZ(rad);
		//Math::Matrix _mat = _transmat * _rad;
...
bool Application::Init(int w, int h)
{
	...
	//// ゲームオブジェクトのリストにハムハムを登録
	//m_GameObjList.push_back(_spHamu);

	m_spHamu = std::make_shared<HamuHamu>();
	m_spHamu->Init();
	...
	m_spTerrain = std::make_shared<Terrain>();
	m_spTerrain->Init();
